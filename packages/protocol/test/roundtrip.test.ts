...

{
  ...
  // Present
  {
    ...
    assert(result.email === 'alice@example.com', `email: expected alice@example.com got ${result.email}`)
  }
...

{
  ...
  for (const tc of testCases) {
    ...
    const result = deserializeInput(layout, buf)
    assert(Math.abs((result.value as number) - tc) < 0.001, `number: expected ${tc} got ${result.value}`)
  }
