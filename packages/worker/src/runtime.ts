...
export class WorkerRuntime {
  ...
  private sendPong(header: Header): void {
    const resp = encodeHeader(
      newHeader(Direction.JSToGo, MessageType.Result, header.requestId, header.handlerId, 200, 0),
    );
  ...
  private async handleDispatch(header: Header, payload: Uint8Array): Promise<void> {
    ...
    if (!entry) {
      const err = encodeHeader(
        newHeader(Direction.JSToGo, MessageType.Error, header.requestId, header.handlerId, 404, 0),
      );
    ...
    try {
      ...
      const outputBuf = entry.outputLayout
        ? serializeOutput(entry.outputLayout, output)
        : new ArrayBuffer(0);
      const resp = encodeHeader(
        newHeader(
          Direction.JSToGo,
      ...
      const err = encodeHeader(
        newHeader(Direction.JSToGo, MessageType.Error, header.requestId, header.handlerId, 500, 0),
      );
      this.socket?.write(new Uint8Array(err));
    }
  }
