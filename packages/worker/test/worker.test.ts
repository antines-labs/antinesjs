...
beforeAll(() => {
  ...
  writeFileSync(
    join(tmpDir, "routes/hello.get.ts"),
    `
export default {
...
test("unknown handlerId returns error", async () => {
  ...
  const result = await new Promise<{ msgType: number; statusCode: number }>(
    async (resolve, reject) => {
      server.on("connection", (goSocket: Socket) => {
        const header = encodeHeader(
          newHeader(Direction.GoToJS, MessageType.Dispatch, 99, 999, 0, 0),
        );
        goSocket.write(new Uint8Array(header));
